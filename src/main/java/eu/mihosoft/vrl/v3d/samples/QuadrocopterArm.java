...
public class QuadrocopterArm {
    ...
    public CSG toCSG() {
        ...
        for (int i = 0; i < 45; i++) {
            CSG cyl = new Cylinder(structureRadius, outerRadius * 10, 8).toCSG();//.optimization(CSG.OptType.NONE);
    ...
    private CSG outerCyl(double outerRadius, double length, double wallThickness) {
        CSG outerCyl = new Cylinder(outerRadius, length, 32).toCSG();//.optimization(CSG.OptType.NONE);
        CSG outerCylInner = new Cylinder(outerRadius - wallThickness, length, 32).toCSG();//.optimization(CSG.OptType.NONE);
