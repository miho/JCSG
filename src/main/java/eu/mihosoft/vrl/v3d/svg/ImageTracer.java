...
public class ImageTracer {
	...
	public static class IndexedImage {
		...
		public IndexedImage(int[][] marray, byte[][] mpalette) {
			...
			height = marray.length - 2;// Color quantization adds +2 to the
	...
	public static IndexedImage colorquantization(ImageData imgd, byte[][] palette, HashMap<String, Float> options) {
		...
		// Creating indexed color array arr which has a boundary filled with -1
		// in every direction
		...
		// Use custom palette if pal is defined or sample or generate custom
		// length palette
		if (palette == null) {
		...
		for (int cnt = 0; cnt < cycles; cnt++) {
			...
			if (cnt > 0) {
				...
				for (int k = 0; k < palette.length; k++) {
					...
					// Randomizing a color, if there are too few pixels and
			...
			for (int j = 0; j < imgd.height; j++) {
				for (int i = 0; i < imgd.width; i++) {
					...
					// find closest color from palette by measuring
					// (rectilinear) color distance between this pixel and all
					// palette colors
					cdl = 256 + 256 + 256 + 256;
					ci = 0;
					for (int k = 0; k < palette.length; k++) {
						...
						cd = c1 + c2 + c3 + (c4 * 4); // weighted alpha seems to
														// help images with
														// transparency
	...
	public static byte[][] generatepalette(int numberofcolors) {
		...
		if (numberofcolors < 8) {
			...
			// RGB color cube
			int colorqnum = (int) Math.floor(Math.pow(numberofcolors, 1.0 / 3.0)); // Number
																					// of
																					// points
																					// on
																					// each
																					...
																					// color
																					// cube
			int colorstep = (int) Math.floor(255 / (colorqnum - 1)); // distance
																		// between
																		// points
			int ccnt = 0;
	...
	// segment[3] , segment[4] : x2 , y2 ; middle point of Q curve, endpoint of
	// L line
	...
	// Z-index
	// the optional lcpr and qcpr are linear and quadratic control point
	// radiuses
	public static String getsvgstring(IndexedImage ii, HashMap<String, Float> options) {
		...
		String viewboxorviewport = options.get("viewbox") != 0 ? "viewBox=\"0 0 " + w + " " + h + "\" "
				: "width=\"" + w + "\" height=\"" + h + "\" ";
	...
	static ImageData blur(ImageData imgd, float rad, float del) {
		...
		for (j = 0; j < imgd.height; j++) {
			for (i = 0; i < imgd.width; i++) {
				...
				idx = ((j * imgd.width) + i) * 4;
				// d is the difference between the blurred and the original
				// pixel
