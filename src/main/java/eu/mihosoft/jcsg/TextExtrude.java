	...
	public static ArrayList<CSG> text(double dir, String text, Font font) {
		...
		while (pi.isDone() == (boolean) false) {
			...
			case PathIterator.SEG_CLOSE:
				points.add( Vector3d.xyz(start[0], start[1], 0));
				...
				if (points.size() > 3) {
					try {
						...
						CSG newLetter = Extrude.points( Vector3d.xyz(0, 0, connectorDepth), points);
			...
			case PathIterator.SEG_QUADTO:
				...
				for (float t = 0.05f; t <= 1.0f; t += 0.05f) {
					...
					points.add( Vector3d.xyz(p1, p2, 0));
					// println "SEG_QUADTO "+p1+" and "+p2
				}
				tmp = coords[2];
				tmp1 = coords[3];
				points.add(Vector3d.xyz(tmp, tmp1, 0));
				break;
